...
class App:
    ...
    def __init__(self, master):
        ...
        self.connecting_entry.grid(row=1, column=2, columnspan=2)

        self.connection_button = Button(root, text="Connect", command=self.connect)
        self.connection_button.grid(row=1, column=4, columnspan=1)

        self.connection_text = StringVar()
        connection_label = Label(root, textvariable=self.connection_text)
        connection_label.grid(row=2, column=0, columnspan=2)

        topic_label = Label(root, text="Subscribe to topic")
        topic_label.grid(row=3, column=0, columnspan=2)

        self.topic_entry = Entry(root, state=DISABLED)
        self.topic_entry.grid(row=3, column=2, columnspan=2)
